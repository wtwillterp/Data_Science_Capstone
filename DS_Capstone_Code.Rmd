...
#Mohawk Industries gcis industry
#ratings %>% filter(gind == 252010)
#Mohawk Industries gcis group
#ratings %>% filter(ggroup == 2520) 
#Mohawk Industries gvkey
#ratings %>% filter(gvkey == 025119)
```
...
NAcheck <- function(df) {
  ...
  for(i in 1:ncol(df)) {       # for-loop over columns
    #which checks for columns that have over 33% of data missing,
    #adding the column names and the respective amount of NAs to vectors
   # if(sum(is.na(df[,i])) > nrow(data)/3){
    names <- append(names, colnames(df[i]))
    percent_of_missing_values <- append(percent_of_missing_values, sum(is.na(df[,i]))/nrow(df))
 # }
  }
  #creating a data frame of the columns with too many missing values and amounts
  output.df <- data.frame(names, percent_of_missing_values)
  #finally outputting it
  output.df
}
...
#function that use quantiles to cap a column's values via IQR method
...
#and it is deemed the high frequency of NAs is the result of data being collected
...
names(stocks)[names(stocks) == 'ï..gvkey'] <- 'gvkey'
stocks$gvkey <- as.character(stocks$gvkey)

#seems like it is mostly demographic information that isn't too useful, however
#maybe ggroup could be useful categorical variable
...
#After doing the likelihood models, where I only used data from Mohawk Industry's gsector
#it was quite apparent that filter was too narrow for the severity model
#consequentially, I need to create a filter to select observations from each data set
#which either are in gsector 25 or have settlement amount data for their gvkey
...
important_tic <- SCA_filings %>% filter(FilingYear > 2009 & FilingYear < 2015) %>% dplyr::select("Ticker")

#across all the data I have trying to find occurrences of those tickers
df <- stocks %>% filter(tic %in% important_tic$Ticker) %>% distinct(gvkey, .keep_all = TRUE) %>% dplyr::select(gvkey, tic, gsector)
df2 <- ratings %>% filter(tic %in% important_tic$Ticker) %>% distinct(gvkey, .keep_all = TRUE) %>% dplyr::select(gvkey, tic, gsector)
df3 <- securities %>% filter(tic %in% important_tic$Ticker) %>% distinct(gvkey, .keep_all = TRUE) %>% dplyr::select(gvkey, tic, gsector)
df4 <- fundamentals %>% filter(tic %in% important_tic$Ticker) %>% distinct(gvkey, .keep_all = TRUE) %>% dplyr::select(gvkey, tic, gsector)

#binding all these occurrences into one data frame
important_gvkey <- rbind(df, setdiff(df2, df))
important_gvkey <- rbind(important_gvkey, setdiff(df3, important_gvkey))
important_gvkey <- rbind(important_gvkey, setdiff(df4, important_gvkey)) %>% distinct(.keep_all = TRUE)

#okay now we have all the important gvkeys, that is gvkeys that correspond to 
#sued companies, this will be useful in the future for joining the other data sets
#to the SCA filings data set
skimr::skim(important_gvkey)

important_gvkey_notgs25 <- important_gvkey %>% filter(gsector != 25) %>% distinct(.keep_all = TRUE)

important_gvkey_gs25 <- important_gvkey %>% filter(gsector == 25) %>% distinct(.keep_all = TRUE)

important_gvkey_gs25

table(important_gvkey$gsector)
#SCA_filings %>% filter(FilingYear > 2009 & FilingYear < 2015) left_join(important_gvkey_gs25, by = c("Ticker" = "tic")) %>% count(gvkey)
```

```{r SCA Fillings Cleaning}
#extracting the useful information
SCA_filings2 <- SCA_filings %>% left_join(important_gvkey, 
                                          by = c("Ticker" = "tic"))
...
#looking at the ratio of gvkeys to Tickers
length(unique(SCA_filings2$gvkey))/length(unique(SCA_filings2$Ticker))
```
...
ratings2 <- ratings %>% 
  #filter(gsector == 25| gvkey %in% important_gvkey$gvkey) %>%
...
#but first since the data occurs over a time span and time series data is tricky
...
#casting a factor as a factor
securities2$curcdm <- as.factor(securities2$curcdm)
summary(securities2$curcdm)
...
#data point per variable per company, so I decided to use mean and
#standard deviations, to capture variation
...
#stocks <- stocks %>% filter(gsector == 25 | gvkey %in% important_gvkey$gvkey)
...
#also removing irrelevant columns like phone, company name, address
...
#log distribution, but ultimately looking at the data a bit it seems plausible 
...
#now I can use this function to summarize the numeric data
#again condensing the date range by taking the mean and standard deviation
...
# #removing columns with excessive NAs after each observation corresponds to
# #a company
...
#also only select companies from the Mohawk industries gsector
...
#could add standard deviations later to capture variance
...
# #assessing NAs before and after removing variables with too many NAs
# fundamentals2 %>% NAcheck() %>% arrange(percent_of_missing_values)
# fundamentals2 <- NAcull(fundamentals2)
# fundamentals2 %>% NAcheck() %>% arrange(percent_of_missing_values)
...
#Joining from most info dense to least ideally
 # Data <- stocks2 %>% full_join(rbind(fundamentals %>% filter(gvkey %in% important_gvkey_gs25$gvkey) %>% distinct(gvkey),
 # stocks %>% filter(gvkey %in% important_gvkey_gs25$gvkey) %>% distinct(gvkey),
 # ratings %>% filter(gvkey %in% important_gvkey_gs25$gvkey) %>% distinct(gvkey),
 # securities %>% filter(gvkey %in% important_gvkey_gs25$gvkey) %>% distinct(gvkey)) %>% distinct(gvkey), by = c("gvkey"))
...
#looking at the amount of the positive case before filtering by gsector
table(Data$Sued)

#removing ticker since it was only used for joining this data frame
Data <- Data %>% 
  #filter(!gvkey %in% important_gvkey_notgs25$gvkey) %>% 
  dplyr::select(-c(Ticker, FilingYear))
#assessing the amount of pos obs
#Data
...
#also setting a seed for reproducible
#results in parallel
...
#change this from doing it on the full data set to just the training set
#since this sequence of feature selection may cause an info leak
...
boruta <- Boruta(Sued ~ ., data = Data[,-c(1,2)], doTrace = 2, maxRuns = 100)
print(boruta)
...
>Predicting the cost of shareholder litigation
...
#tuning the model using ten fold cross validation
lr_tune_results <- 
  Data_wf %>%
  add_model(lr_spec) %>% 
...
#while n is the number of trees to build (from which classification is decided)
rf_tune_results <-
  Data_wf %>%
  add_model(rf_spec) %>% 
  tune_grid(Data_folds,
...
```{r knn_model}
#probably necessary for random tie-breaking
set.seed(345)
...
#tuning the model
#only thing to tune is the # of neighbors
knn_tune_results <- 
  Data_wf %>%
  add_model(knn_spec) %>% 
  tune_grid(Data_folds,
...
#lr_tune_results %>% collect_metrics() %>% 
# dplyr::filter(.config == lr_tune_results %>% 
#                 select_best(metric = "kap") %>% pull()) %>% mutate(model = "Logistic Regression")
...
#seems like they are all fairly close
#but AUC is not the best comparison when we are more concerned about 
#more than just the classification ranking
#but the accuracy of the likelihood probabilities underlying them
...
Like_mdl_met_train_ll <- bind_rows(lr_tune_results %>% collect_metrics(summarise = TRUE) %>% 
                                 ...
                                 knn_tune_results %>% collect_metrics(summarise = TRUE) %>% dplyr::filter(.config == knn_tune_results %>% select_best("mn_log_loss") %>% pull()) %>% mutate(model = "K-Nearest Neighbors")
                                 )
...
#.218 is the non-informative value of log loss
...
#it is clear random forest is a much better classifier
Like_mdl_met_train_ll %>% 
...
# create a dataframe with all the best models metrics
...
final_xgb_test_results %>%
  ...
  conf_mat(Sued, .pred_class) %>%
  autoplot(cm, type = "heatmap") +
  labs(title = "Random Forest Confusion Matrix on Test with Optimal F2 Measure as Threshold") +
  scale_fill_stepsn( colors = c("white", "#00d0d8"), breaks = c(15, 40, 100)) +
  theme(panel.border = element_rect(colour = "#00d0d8", fill=NA, size= 1))
...
#   conf_mat(Sued, .pred_class) %>%
#   autoplot(cm, type = "heatmap") +
#   labs(title = "Random Forest Confusion Matrix on Test with CV BAcc as Threshold") +
#   scale_fill_stepsn( colors = c("white", "#00d0d8"), breaks = c(15, 40, 100)) +
#   theme(panel.border = element_rect(colour = "#00d0d8", fill=NA, size= 1))
...
#plotting the CM of the test set pred of the final model
final_lr_test_results %>%
  collect_predictions() %>%
  #custom threshold
  mutate(.pred_class = ifelse(.pred_Yes > lr_best_thresh, "Yes", "No")) %>%
  conf_mat(Sued, .pred_class) %>%
  autoplot(cm, type = "heatmap") +
  labs(title = "LR Confusion Matrix on Test with Optimal F2 Measure as Threshold") +
  scale_fill_stepsn( colors = c("white", "#7CAE00"), breaks = c(15, 40, 100)) +
  theme(panel.border = element_rect(colour = "#7CAE00", fill=NA, size= 1))

#predicting Mohawk Industries likelihood
#actually a pseudo probability, fix later
final_rf_model <- fit(final_rf, Data_train)
predict(final_rf_model, Mohawk_Obs, type = "prob") %>% mutate(model = "RF")
...
#final_lr_model <- fit(final_lr, Data_train)
...
```{r RF Calibration Assessment and Adjustment}
...
#creating data for calibration using monte carlo cross validation
#see the paper: Getting More Out of Small Data Sets
#Improving the Calibration Performance of Isotonic Regression by Generating More Data
#https://www.scitepress.org/papers/2018/65760/65760.pdf
# lr_mccv <- final_lr %>% 
#   fit_resamples(resamples = Data_cal_folds, control = control_resamples(save_pred = TRUE, save_workflow = TRUE))
# lr_mccv <- lr_mccv %>% collect_predictions()

# rf_cv <- final_rf %>% 
#   fit_resamples(resamples = Data_folds, control = control_resamples(save_pred = TRUE, save_workflow = TRUE))
# rf_cv <- xgb_cv %>% collect_predictions()
...
#Dat_sv <- SCA_filings2 %>% filter(SettlementAmount > 0) %>% add_row(gvkey = "025119")
...
#Adding the final variables of interest to the dataframe, settlement amount
...
#tentative rough fix fixes features that aren't 
...
#features_selected_sv <- features_selected_sv[! features_selected_sv %in% c('dd3_avg')]
...
#Dat_sv_rec <- recipe(SettlementAmount ~ ., data = Dat_sv[,features_selected]) %>%
...
Sv_mdl_outer_errors %>% relocate(model) %>% arrange(rmse)
...
```{r Best Severity Prediction XGB}
...
final_XGBoost_sv_tune_results <- Dat_sv_wf %>%
    add_model(xgb_sv_spec) %>%
...
#finalizing work flow
final_best_XGBoost_sv_wf <- finalize_workflow(XGBoost_sv_wf, final_XGBoost_sv_tune_results %>% select_best("rmse"))

#fitting the final model with optimized hyper-parameters
final_fit_best_XGBoost_sv_wf <- fit(final_best_XGBoost_sv_wf, data = Dat_sv[,features_selected_sv])

#make a nice feature importance plot
final_fit_best_XGBoost_sv_wf %>%
  pull_workflow_fit() %>%
  vip(geom = "point")

#final prediction
predict(final_fit_best_XGBoost_sv_wf, Mohawk_Obs_sv[,features_selected_sv])

#oh god I need a confidence interval oops
#we'll use the naive magpie method

pred_quantiles <- final_XGBoost_sv_tune_results %>% collect_predictions() %>% mutate(residual = abs(SettlementAmount-.pred)) %>% dplyr::select(residual) %>% pull() %>% quantile(probs = c(.9, .95))



bind_rows(Lower = predict(final_fit_best_XGBoost_sv_wf, Mohawk_Obs_sv[,features_selected_sv]) - pred_quantiles[2], Upper = predict(final_fit_best_XGBoost_sv_wf, Mohawk_Obs_sv[,features_selected_sv]) + pred_quantiles[2])

final_XGBoost_sv_tune_results %>% collect_predictions() %>% mutate(residual = abs(SettlementAmount-.pred)) %>%
  ggplot(aes(residual)) +
  geom_histogram(color = "#000000", fill = "#85C285", bins = 20) +
  #geom_text(data = annotations, aes(x = x, y = y, label = paste(label, x)), size = 5, fontface = "bold") +
  scale_x_continuous(labels=scales::dollar_format()) +
  geom_vline(xintercept= pred_quantiles[2], 
             ...
             color = "red",
             size = .5) +
  labs(title = "Absolute Calibration Errors for the Final XGBoost Model", caption = "Red line indicates .95 quantile") +
  theme_classic()
```

```{r Best Severity Prediction RF}
#now according to Raschka we refit the best algorithm on the whole data set
#with hyper parameters optimized on cross validation using the whole data set
#and predict 
...
#oh god I need a confidence interval oops
#we'll use the naive mapie method
