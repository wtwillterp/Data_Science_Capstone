...
```{r Stocks Cleaning, warning=FALSE}
...
rf_spec <- rand_forest(mtry = tune(), min_n = tune()) %>%
  ...
  set_engine("randomForest", num.threads = (parallel::detectCores() - 1)) %>% 
...
#performing the last fit
final_rf_test_results <- last_fit(final_rf, Data_split, metrics = metrics)

# collecting the final predictions
collect_metrics(final_rf_test_results)
```
...
```{r Final Model Calibration Adjustment}
...
#it has issues for infrequent events but is better for interpretability
# summary(verify(obs = xgb_cv$Sued_1hot, pred = xgb_cv$.pred_Yes))
...
```{r Final Model Calibration Assessment on Test}
...
#truth_var is the variable in pred_data that contains the one hot encoded true class
#plot_title is the title of the plot
plot_cal_curve <- function(pred_data, pred_var, truth_var, plot_title){

  #creating the bins and standard errors
  cal_plot <- dplyr::mutate(pred_data, bin = dplyr::ntile(!!rlang::parse_expr(pred_var), 10)) %>% 
  dplyr::group_by(bin) %>% 
  dplyr::mutate(n = dplyr::n(), 
                bin_pred = mean(!!rlang::parse_expr(pred_var), na.rm = TRUE), 
                bin_prob = mean(as.numeric(as.character(!!rlang::parse_expr(truth_var)), na.rm = TRUE)), 
  ...
  ggplot2::geom_errorbar(ggplot2::aes(x = bin_pred, y = bin_prob, ymin = ll, ymax = ul), 
                         ...
                         color = "black", 
                         width = 0.02) + 
  ggplot2::geom_smooth(ggplot2::aes(x = !!rlang::parse_expr(pred_var), y = as.numeric(as.character(!!rlang::parse_expr(truth_var)), na.rm = TRUE), color = "#00BFC4"), 
                       se = TRUE, 
                       method = "loess") + 
...
#calibration curves via cross validation
plot_cal_curve(xgb_cv, ".pred_Yes", "Sued_1hot", "XGBoost Calibration curve (Uncalibrated)")
plot_cal_curve(xgb_cv, "platt_Yes", "Sued_1hot", "XGBoost Calibration curve (Platt)")
plot_cal_curve(xgb_cv, "iso_Yes", "Sued_1hot", "XGBoost Calibration curve (Isotonic)")
plot_cal_curve(xgb_cv, "beta_Yes", "Sued_1hot", "XGBoost Calibration curve (Beta)")

#calibration curve on test
plot_cal_curve(xgb_test_res, ".pred_Yes", "Sued_1hot", "XGBoost Calibration curve on test (Beta Calibration)")
```
